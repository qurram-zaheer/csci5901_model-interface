...
BASE_URL = "http://ec2-44-201-255-228.compute-1.amazonaws.com:8501/v1/models/model"

with tab1:
    st.header("Model interface")

    status_dict = list(requests.get(f'{BASE_URL}').json().values())[0][0]
    ...
    if number:
        ...
        json_response = requests.post(f'{BASE_URL}:predict', data=data, headers=headers)
